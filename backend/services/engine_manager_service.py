...
try:
    ...
    from ..vocal_separator import AUDIO_PIPELINE_DIR, AUDIO_PIPELINE_VENV, UVR5_MODEL_PATH
except ImportError:
    from services.model_service import list_models
    from voice_changer import (
        COVER_INFERENCE_MODE,
...
ENGINE_KEYS = {"rvc_webui", "uvr", "svc_fallback"}
...
def list_rvc_models(
    ...
    *,
    ...
    engine = get_rvc_engine(project_root, weights_dir, force=force)
...
def _get_cached_rvc_models(
    ...
    *,
    ...
    now = time.monotonic()
    root_key = "|".join(str(item) for item in [root, weight_root, index_root, logs_root])
    if (
        not force
    ...
    items = _scan_rvc_models(root, weight_root, index_root, logs_root, project_root, weights_dir)
...
def _discover_online_rvc_base() -> str:
