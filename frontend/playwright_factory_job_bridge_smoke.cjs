...
const REMOTE_DEBUG_PORT = 9237;
const SCREENSHOT_PATH = path.join(__dirname, "..", "stage21_factory_bridge.png");
...
function launchChrome() {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "feishark-stage21-factory-"));
  const chrome = spawn(
    CHROME_PATH,
    [
      "--headless=new",
...
async function pickUsableModel() {
  ...
  if (!usable?.model_id) {
    throw new Error("No usable cover model available for Stage 21 smoke");
  }
  return usable;
}
...
async function createBatch() {
  return fetchJson(`${BASE_URL}/api/batches`, {
    ...
    body: JSON.stringify({
      batch_name: `stage21_factory_bridge_${Date.now()}`,
      target_platforms: ["douyin", "bilibili"],
      metadata: { smoke: true, source: "playwright_factory_job_bridge_smoke" },
    }),
  });
}
...
async function importTracks(batchId) {
  ...
  formData.append("files", new Blob([fs.readFileSync(AUDIO_A)], { type: "audio/wav" }), "stage21-bridge-a.wav");
  formData.append("files", new Blob([fs.readFileSync(AUDIO_B)], { type: "audio/wav" }), "stage21-bridge-b.wav");
  formData.append("titles_json", JSON.stringify(["stage21_bridge_track_a", "stage21_bridge_track_b"]));
  formData.append("artist", "stage21_bridge_artist");
  formData.append("notes", "playwright bridge smoke");
  formData.append("metadata_json", JSON.stringify({ smoke: true, source: "playwright_factory_job_bridge_smoke" }));
...
async function selectBatchAndTrack(client, batchId, trackId) {
  ...
  `);
  await client.waitFor(
    `document.getElementById("factoryTrackTitle")?.textContent?.includes(${JSON.stringify(trackId)}) || document.getElementById("factoryTrackSubtitle")?.textContent?.includes("stage21_bridge_track_b")`,
    20000,
...
async function waitForTrackJob(client, trackId, timeoutMs = 120000) {
  const deadline = Date.now() + timeoutMs;
  let latest = null;
  while (Date.now() < deadline) {
...
async function maybeWaitForCompletedJob(trackId, timeoutMs = 120000) {
  const deadline = Date.now() + timeoutMs;
  let latest = null;
  while (Date.now() < deadline) {
    latest = await fetchTrackJobs(trackId);
    ...
    if (item?.status && item.status !== "pending") {
      if (item.can_open_studio || item.final_artifact_download_url) {
        return item;
      }
    }
    await delay(3000);
...
async function verifyTrackJobUi(client, jobId) {
  await client.waitFor(
    `Boolean(document.querySelector('#factoryTrackJobsList [data-track-job-id="${jobId}"]'))`,
    20000,
    `track job ${jobId} visible`,
  );
}

async function verifyCompletedActionsIfReady(client, completedJob) {
  if (!completedJob?.job_id || !completedJob.can_open_studio) return;
  await client.evaluate(`document.getElementById("factoryReloadTrackBtn")?.click();`);
  await client.waitFor(
    `Boolean(document.querySelector('#factoryTrackJobsList [data-track-job-id="${completedJob.job_id}"]'))`,
    20000,
    "reloaded track job row",
  );
  await client.waitFor(
    `Boolean(document.querySelector('#factoryTrackJobsList [data-track-job-id="${completedJob.job_id}"] [data-studio-url]'))`,
    20000,
    "Studio action visible",
  );
  await client.waitFor(
    `Boolean(document.querySelector('#factoryTrackJobsList [data-track-job-id="${completedJob.job_id}"] [data-download-url]'))`,
    20000,
...
async function main() {
  ...
  try {
    if (!fs.existsSync(AUDIO_A) || !fs.existsSync(AUDIO_B)) {
      throw new Error(`Stage 21 audio fixtures missing: ${AUDIO_A} / ${AUDIO_B}`);
    }
    ...
    const relatedJob = await waitForTrackJob(client, trackId);
    await verifyTrackJobUi(client, relatedJob.job_id);

    const maybeCompleted = await maybeWaitForCompletedJob(trackId);
    await verifyCompletedActionsIfReady(client, maybeCompleted);

    await client.screenshot(SCREENSHOT_PATH);
    console.log("FACTORY_JOB_BRIDGE_PLAYWRIGHT_SMOKE PASS");
    console.log(`Screenshot: ${SCREENSHOT_PATH}`);
    console.log(`Track: ${trackId}`);
    console.log(`Job: ${relatedJob.job_id}`);
    console.log(`Status: ${maybeCompleted?.status || relatedJob.status || "pending"}`);
  } finally {
    client?.close();
...
main().catch(error => {
  console.error("FACTORY_JOB_BRIDGE_PLAYWRIGHT_SMOKE FAIL");
